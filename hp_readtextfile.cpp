...
float HP_ReadTextFile::StandardDeviation(float* values, int number_of_values)
{
float average = 0;
float sum = 0;
...
for(int x = 0; x < number_of_values; x++)
	sum_of_squares += pow((values[x]-average),2);
float normalised_sum_of_squares = sum_of_squares/(number_of_values-1);
float std = sqrt(normalised_sum_of_squares);
return(std);
}
